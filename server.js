...
const app = express();
const PORT = 3000;
const JWT_SECRET = 'kazuchar_secret_key_2024_secure';

const db = new sqlite3.Database('kazuchar.db');
...
['uploads/profiles', 'uploads/characters', 'uploads/chat'].forEach(dir => {
    if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
});

const storage = multer.diskStorage({
    destination: (req, file, cb) => cb(null, 'uploads/' + (req.query.type || 'profiles')),
    filename: (req, file, cb) => cb(null, Date.now() + '-' + Math.round(Math.random() * 1E9) + path.extname(file.originalname))
});
const upload = multer({ storage, limits: { fileSize: 10 * 1024 * 1024 } });
const chatUpload = multer({ storage: multer.diskStorage({ destination: 'uploads/chat', filename: (req, file, cb) => cb(null, Date.now() + '-' + Math.round(Math.random() * 1E9) + path.extname(file.originalname)) }), limits: { fileSize: 20 * 1024 * 1024 } });
...
app.listen(PORT, () => {
    console.log('KazuChar.AI running on http://localhost:' + PORT);
